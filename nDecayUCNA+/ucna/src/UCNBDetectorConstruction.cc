...
//  double T_MWPC = 298*kelvin;
  ...
  double T_MWPC = 298*kelvin;
  double P_MWPC = 100*torr;
 // G4Material* WCPentane = NULL;

 G4Material* WCPentane = new G4Material("Pentane",(72.17*mg)/(22.4*cm3)*P_MWPC/(760*torr)*(273.15*kelvin)/T_MWPC,2,kStateGas,T_MWPC,P_MWPC);
 WCPentane->AddElement(C, fractionmass=0.832);
 WCPentane->AddElement(H, fractionmass=0.168);
...
/*need to place within the logical volume of the drift tu*/
 ...
 // G4double zFoil =  1.6*m  ;
  ...
  G4double thicknessDead = 3e-06*m;
 ...
 G4double zDead = halfLengthDrift12 - thicknessDead/2 ;     
 G4double zDead2 = (-halfLengthDrift12+ thicknessDead/2);     
 ...
 physicalDead1 = new G4PVPlacement(0, G4ThreeVector(xDead,yDead,zDead),
                                    logicalDead1, "Dead1", logicalDrift1, false, 0);

 //G4double zDead2 = ( si_dis +dMWPCThick+ tDead/2.0)*m;   
 ...
 physicalDead2 = new G4PVPlacement(0, G4ThreeVector(xDead,yDead,zDead2),
				    logicalDead2, "Dead2", logicalDrift2, false, 0);

  G4double dMWPCThick = 0.6e-06*m ;
  G4double zMWPC  = zDead - thicknessDead/2 -dMWPCThick/2 ;
  G4double zMWPC2  = zDead2 + thicknessDead/2 -dMWPCThick/2 ;
  mwpc1 = new G4Tubs("mwpc1", 0., outerRadiusDecayTrapInt, dMWPCThick/2., 0., 2*M_PI);
  logicalmwpc1 = new G4LogicalVolume(mwpc1, Mylar, "mwpc1");
  physicalmwpc1 = new G4PVPlacement(0, G4ThreeVector(0,0,    zMWPC), logicalmwpc1, "mwpc1", logicalDrift1, false, 0);
 
  mwpc2 = new G4Tubs("mwpc2", 0., outerRadiusDecayTrapInt, dMWPCThick/2., 0., 2*M_PI);
  logicalmwpc2 = new G4LogicalVolume(mwpc2, Mylar, "mwpc2");
  physicalmwpc2 = new G4PVPlacement(0, G4ThreeVector(0,0,    zMWPC2), logicalmwpc2, "mwpc2", logicalDrift2, false, 0);
  
  G4cout<<"zMWPC 1   : "<<zMWPC<<G4endl;
  G4cout<<"zMWPC 2   : "<<zMWPC2<<G4endl;

  G4double dThicknessGas = 3.00e-02*m;
  G4double zPentane =  (zMWPC - dMWPCThick/2- dThicknessGas/2) ; 

  pentaneVol    = new G4Tubs("pentaneVol", 0., outerRadiusDecayTrapInt, dThicknessGas/2., 0., 2*M_PI);
  logicalpentaneVol = new G4LogicalVolume(pentaneVol,WCPentane, "pentaneVol");
  physicalpentaneVol = new G4PVPlacement(0, G4ThreeVector(0,0,    zPentane), logicalpentaneVol, "pentaneVol", logicalDrift1, false, 0);
  G4cout<<" Placement of the gas : "<<zPentane<<G4endl;


  G4double zPentane2 =  (zMWPC2 + dMWPCThick/2+ dThicknessGas/2) ; 
  
  pentaneVol2    = new G4Tubs("pentaneVol2", 0., outerRadiusDecayTrapInt, dThicknessGas/2., 0., 2*M_PI);
  logicalpentaneVol2 = new G4LogicalVolume(pentaneVol2,WCPentane, "pentaneVol2");
  physicalpentaneVol2 = new G4PVPlacement(0, G4ThreeVector(0,0,    zPentane2), logicalpentaneVol2, "pentaneVol2", logicalDrift2, false, 0);
  G4cout<<" Placement of the gas : "<<zPentane2<<G4endl;

  G4double zMWPCEnter  = zPentane - dThicknessGas/2 - dMWPCThick/2 ;
  G4double zMWPC2Enter  =zPentane2 + dThicknessGas/2 + dMWPCThick/2;

  mwpc1_enter = new G4Tubs("mwpc1_enter", 0., outerRadiusDecayTrapInt, dMWPCThick/2., 0., 2*M_PI);
  logicalmwpc1_enter = new G4LogicalVolume(mwpc1_enter, Mylar, "mwpc1_enter");
  physicalmwpc1_enter = new G4PVPlacement(0, G4ThreeVector(0,0,    zMWPCEnter), logicalmwpc1_enter, "mwpc1_enter", logicalDrift1, false, 0);
 
  mwpc2_enter = new G4Tubs("mwpc2_enter", 0., outerRadiusDecayTrapInt, dMWPCThick/2., 0., 2*M_PI);
  logicalmwpc2_enter = new G4LogicalVolume(mwpc2_enter, Mylar, "mwpc2_enter");
  physicalmwpc2_enter= new G4PVPlacement(0, G4ThreeVector(0,0,    zMWPC2Enter), logicalmwpc2_enter, "mwpc2_enter", logicalDrift2, false, 0);

  G4cout<<"zMWPC 1  enter : "<<zMWPCEnter<<G4endl;
  G4cout<<"zMWPC 2  enter : "<<zMWPC2Enter<<G4endl;
...
//container_log_2 = new G4LogicalVolume(N2VolTube, WCNitrogen, Append(side, "N2_Vol_log_"));
//  container_log_2 -> SetVisAttributes(G4VisAttributes::Invisible);
...
//placement of container in the logical volume

//G4double container_log_z = ( (si_dis)+ dN2Volume_Z/2)*m;
//container_log_phys =  new G4PVPlacement(0, G4ThreeVector(0,0,container_log_z), container_log, "container_log_phys", logicalWorld, false, 0);
 
//container_log_phys_2 =  new G4PVPlacement(0, G4ThreeVector(0,0,-1*container_log_z), container_log_2, "container_log_phys_2", logicalWorld, false, 0);
//-------------------------ooooOOOOOOOOO0000OOOOOOoooooooo----------------------------------------
