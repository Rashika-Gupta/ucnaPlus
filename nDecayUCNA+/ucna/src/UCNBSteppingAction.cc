...
//--------------Energy deposition in Silicon detectors and deadlayers-----------------------------------------
 ...
 if((fStep->GetPreStepPoint()->GetPhysicalVolume()->GetName() == "mwpc1_enter" && fStep->GetPostStepPoint()->GetPhysicalVolume()->GetName() == "pentaneVol1") || 
  (fStep->GetPreStepPoint()->GetPhysicalVolume()->GetName() == "pentaneVol1" &&
  fStep->GetPostStepPoint()->GetPhysicalVolume()->GetName() == "mwpc1") ) {
   	  if(fStep->GetTrack()->GetTrackID()==1){   
      G4cout<<" Entering : "<<  fStep->GetPostStepPoint()->GetPhysicalVolume()->GetName()<<G4endl;
      ...
      if ( ((fStep->GetTrack()->GetTrackID() != 2)) && ((fStep->GetTrack()->GetParentID() != 2)) ) {
        G4double dEstep = fStep->GetTotalEnergyDeposit();
        UCNBAnalysisManager::getInstance()->AddUpElectronMWPC1EnergyDeposition(dEstep/keV);
        }}
  }

  if (
  (fStep->GetPreStepPoint()->GetPhysicalVolume()->GetName() == "mwpc2_enter" && fStep->GetPostStepPoint()->GetPhysicalVolume()->GetName() == "pentaneVol2") || 
  (fStep->GetPreStepPoint()->GetPhysicalVolume()->GetName() == "pentaneVol2" &&
  fStep->GetPostStepPoint()->GetPhysicalVolume()->GetName() == "mwpc2" )) {
      if(fStep->GetTrack()->GetTrackID()==1){   
      ...
      if ( ((fStep->GetTrack()->GetTrackID() != 2)) && ((fStep->GetTrack()->GetParentID() != 2)) ) {
        G4double dEstep = fStep->GetTotalEnergyDeposit();
        UCNBAnalysisManager::getInstance()->AddUpElectronMWPC2EnergyDeposition(dEstep/keV);
        }}
  }
